"""
阶段二 Day4（9/17）滚动回测 + Kupiec LR_uc + Christoffersen LR_ind —— 双口径 × 95%/99% × 多范围

回答 spec §7.2 明列的两问：**整体覆盖率准不准（LR_uc）+ 违规会不会扎堆（LR_ind）**。
9/15（参数法）与 9/16（历史模拟法）只出 VaR 序列、不出检验；按 spec §6 L157 的分工，
所有检验统一由本脚本出具，以免出现两套官方数字。

输入：results/var_parametric.csv (9/15)  results/var_historical.csv (9/16)
      results/regimes.csv  results/doubtful_days.csv  results/garch_refit_trace_rmb.csv
      events/risk_events_timeline.csv  factors/factor_table_3141HK.csv
产出：results/backtest_results.csv    每模型 × 口径 × 置信度 × 范围一行的完整检验长表
      results/backtest_exceptions.csv 逐条违规判定表（含存疑/事件/跳变近邻标记）
      figures/backtest_exceptions.png 违规时间线 + 数据驱动的事件标注
      figures/backtest_coverage.png   失败率点估计 + Kupiec 精确接受区间（检验力可视化）

口径纪律（spec §7）：
  · 违规判定统一走 `var_common.hit_series`，与 9/16 的预览计数共用同一条 `r_t < −VaR_t`
    —— 两处各写一份是本模块存在的原因（防口径分叉）。
  · **M1δ（δ-normal）是代数恒等式**，与 M1 的 x/N 必然相同（spec §4 L102-103），
    `role` 列标注，**不得作为独立模型证据**。
  · 分段标志 `regimes.csv` 是含当日的 60 日滚动 σ，属**事后同期标记**，不是可交易信号；
    报告该段失败率时必须写明，不得称「事前可知的高波动段」（spec §7.3 L267-269）。
  · χ²(1) 是渐近近似，本样本（99% 下期望违规仅约 10 次）不满足。故 LR_uc / LR_ind 除 χ² p 外
    并列给出**蒙特卡洛有限样本 p**，并据同一零分布披露名义 5% 的实际水平（size 表）。
    LR_ind 另给**条件版** p（以观测违规数为率模拟），它回答的正是「给定违规次数，是否扎堆」。

用法： ./.venv/bin/python code/var_backtest.py
      （须先跑 var_parametric.py 与 var_historical.py，复现顺序见 spec §9）
"""
...
RES, FIG = REPO / "results", REPO / "figures"
...
EVT = pd.read_csv(REPO / "events" / "risk_events_timeline.csv", encoding="utf-8-sig", parse_dates=["date"])
B31 = pd.read_csv(REPO / "factors" / "factor_table_3141HK.csv", encoding="utf-8-sig", parse_dates=["date"]).set_index("date")
SRC = {"par": VP, "his": VH}
