...
- **[工具使用说明（环境搭建 / 一键与分阶段运行 / 脚本职责 / 输出物清单 / 图-脚本-数据对应 / 硬断言语义 / 已知边界 / 典型调用序列 / 常见报错与处理）](docs/tool_usage.md)**
...
code/        数据获取 / 清洗 / 建模代码
tools/       开发期校验脚本（不进生产链路；ast_guard.py 用于证明批量改写注释未动可执行代码）
figures/     可视化图表
docs/        数据说明文档与研究文档
```
...
三路径映射方法、缓冲量级来源、风险承受能力评估、**六项稳健性检验**与**方法局限**见
...
| `stress_factor_contrib.png` | 因子贡献度分解（残差以纹理单列，不在因子之间摊分；残差为拟合误差而非信用风险贡献，纯汇率情景次口径下的 100% 系口径定义） | 9/23 |
