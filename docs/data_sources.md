...
| 2 | 标的行情 | iShares 中国投资级美元债 ETF | Yahoo Finance | MCHB | 日 | **待取**（Yahoo 今日限流，见下） |
...
3. **MCHB（Yahoo）今日返回 429 限流 / 反爬页**，属临时性访问限制，非代码问题。
...
## 三、MCHB 手动替代方案（Yahoo 被限流时）

1. 浏览器打开 https://finance.yahoo.com/quote/MCHB/history/
2. 时间范围设 **2021-08-01 ~ 今天**、频率 **Daily**，点 **Download**
3. 保存为 `raw_data/mchb.csv`（第一列 `Date`，含 OHLC/Adj Close/Volume）
