...
具体分为五部分：一是构建压力情景库，含 3 个历史情景、2 个补充情景与 10 个假设情景；二是为利率、信用、汇率三类冲击分别建立到组合损失的映射路径，三条路径不共用同一套系数；三是完成潜在损失与最大回撤测算，并以 1 日 99% 期望损失为判定线识别尾部风险点；四是做因子贡献度分解，把无法归因的部分单列为残差；五是做六项稳健性检验，并据此把结论分成「稳健」与「不稳健」两档。最终形成需求文档要求的两项交付物：《压力情景库》与《压力测试分析报告》。在收到 9 月反馈后，本周另按反馈完成三项口径修订；此后按老师指示把主情景库的持有期统一到 1 日，重做全库并重跑全部测算。因此**本报告的全部损失读数、判定阈值与结论均为 1 日持有期口径**。

本周的核心结论有五条，**全部按 1 日持有期口径给出**（重做理由与来源见 §五 第 11 条）。其中三条的方向与上一版相反——缓冲敏感性、信用口径依赖、次口径尾部方向，翻转前后的逐条对照见《压力测试分析报告》**§一 结论前置**，机制见 §五 第 12 条；以下只给当前读数。
...
第五，六项稳健性检验中，两项支持「稳健」、三项要求带条件引用；新增的第六项（阈值口径交叉核对）并入判定名单一条讨论。与上一版的比例相同但成员有变。相对稳健的两项是：**1 日口径下的库级覆盖成立**，以及**历史情景的损失比多期口径更抗窗口平移**（十个「情景 × 口径」组合中有三个读数在 ±2 日平移下完全不动——H1 双口径与 H2 主口径）。必须带条件引用的三项是：**假设情景的绝对水平**高度依赖锚点窗长（极端档极差：利率 1.8262pp、汇率 3.9580pp），**历史情景的个别读数**仍会随窗口移动（H3 主口径极差 0.5766pp），以及**尾部判定名单的边际成员**——超限条数随缓冲档位由 17 条升至 23 条、换用交叉核对阈值线后由 19 条降为 15 条，与上一版「名单不随缓冲改变」的结论相反（详见 §2.6）。其中缓冲敏感性这一条是本轮最需要提示使用者的：判定名单对缓冲的档位选择敏感，故应以「相当于几倍 99% 期望损失」的连续读数为准，不宜只看命中的条数。
...
表 3-5 有两点必须说明。其一，**交叉核对列比主判定线大 29% ~ 52%**（主口径 0.8573 vs 0.6659，高 28.7%；次口径 1.0646 vs 0.7024，高 51.6%）。差异来源是样本与统计量两重不同：主判定线来自滚动 250 日窗口，随波动状态变化，且是「已违规日的平均损失」这一条件量；交叉核对来自 2021-08 起的全样本无条件分位数。二者都能各自辩护，本报告取前者为主判定线（因其是阶段二登记口径），但**判定对阈值口径敏感**——同一份损失改用交叉核对列作阈值，超限条数由 19 条降为 15 条。该敏感性的逐条对照按老师要求收在**稳健性部分**（§2.6 表 3-8 及表下说明）。
...
**引用表 3-7 与图 3-4 时有两个边界必须一并给出。** 一是**未解释残差是模型的拟合误差，不等于信用风险贡献**：残差占比高主要受信用因子数据覆盖不足（`doas_bp` 自 2023-09-06 才有值，2022 年信用冲击无数据可映射）与窗口边界截断（信用走阔的前半段落在窗口之外）的影响，不代表真实信用风险的高低。二是**纯汇率情景在次口径下汇率贡献 100% 是口径定义的结果**，不代表组合里汇率风险占比这么高；多因子历史情景的汇率贡献大致在 **20%–25%** 区间（H1 为 25.0%、H3 为 24.9%），其余情景因因子构成不同而偏离（H2 为 10.4%、X1 为 66.4%、X2 为 4.0%）。

**图 3-4 因子贡献度分解**

![图 3-4 因子贡献度分解（残差为拟合误差、非信用风险贡献；纯汇率情景 100% 系口径定义）](../figures/stress_factor_contrib.png)

图 3-4 数据来源：`results/stress_factor_contrib.csv`。残差以纹理单列，不与因子混计。**纹理段为拟合误差而非信用风险贡献；纯汇率情景次口径下的 100% 系口径定义，多因子历史情景的汇率贡献在 20%–25% 区间。**
...
**具体实施**：共做六项检验——情景覆盖度、窗口敏感性、缓冲敏感性、信用区间敏感性、锚点窗长敏感性、阈值口径交叉核对。前三项分别回答「情景库够不够深」「历史情景的窗口起止差一天会怎样」「缓冲取多大才影响结论」；后三项回答「信用结论对传导系数取值的敏感度」「假设情景的绝对水平对锚点窗长的敏感度」与「换一条同样合理的判定阈值，超限名单会怎么变」。

**实际结果**：

**表 3-8 六项稳健性检验结论（1 日口径）**
...
表 3-8 数据来源：`results/stress_buffer_sens.csv`（150 行：5 档缓冲 × 30 情景—口径）、`results/stress_coverage.csv`（30 行）、`results/stress_window_sens.csv`（50 行：5 情景 × 5 平移 × 2 口径）、`results/stress_anchor_sens.csv`（12 行）、`results/stress_shift_coverage.csv`（8 行）。前五项由 `code/stress_robustness.py` 产出；**第六项不另出表**，直接取自 `results/stress_impact.csv` 的 `es_1d_99_pct` / `es_hd_99_pct` 与 `tail_flag` / `tail_flag_hd` 两对列。**五张表均为本次 1 日重做后重算**（`stress_coverage.csv` 的 `horizon_days` 现全为 1，其 `loss_cum_pct` 列仍保留窗内累计值供方向性陈述使用）。
...
**阈值口径交叉核对**这一行在本次修订中新增。主判定线与交叉核对线相差 29% ~ 52%，两者不是同一个统计量：前者是滚动 250 日窗口的**条件**期望损失（违规日的平均实际损失，仅由 8（主）/ 7（次）个违规日支撑），后者是全样本**无条件**经验分位数。改用后者作阈值，30 条组合的超限条数由 **19 条降为 15 条**，被移出的 4 条是 H1 次口径（0.8019 < 1.0646）、H3 次口径（0.7680 < 1.0646）、S06 主口径（0.7536 < 0.8573）与 S06 次口径（0.7536 < 1.0646），都是贴着阈值的情景。**方向性结论不受影响**：汇率情景 S07–S09 的次口径损失在两条线下都超限，S10 仍是主口径最深情景。本报告取阶段二登记口径为主判定线——阶段二 VaR 与第三阶段压力测试共用同一条参照线，情景损失才能与日常风控实际使用的 VaR 口径直接对接。
...
1. ~~尾部判定对阈值口径敏感~~ **（已于 9/26 定案，结项）**。老师确认按阶段二登记的滚动 250 日条件期望损失作主判定线（主口径 0.6659%、次口径 0.7024%），理由是阶段二 VaR 与第三阶段压力测试共用同一条参照线，情景损失才能与日常风控实际使用的 VaR 口径对接，压力测试结果才有进入风控流程的可能。全样本无条件分位数（0.8573% / 1.0646%）保留作交叉核对。换线对超限名单的影响（19 条降为 15 条）按老师要求改挂在**稳健性部分**披露，见 §2.6 表 3-8 及其表下说明。
...
- 因子贡献度分解，未解释残差单列
- 六项稳健性检验：覆盖度、窗口、缓冲、信用区间、锚点窗长、阈值口径交叉核对
- 情景覆盖度检验并据此补入补充情景 X2
