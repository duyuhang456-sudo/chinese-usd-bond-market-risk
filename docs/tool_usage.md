...
| `--only <阶段>` | 只跑指定阶段，逗号分隔。合法取值只有 `phase1` / `phase2` / `phase3`，其它一律返回退出码 2 |
...
- `0`：全部步骤成功，且跑完后的产物对账通过。
- `1`：计算类脚本失败，或产物对账不通过。取数类脚本失败只记警告，不影响退出码。
- `2`：`--only` 里出现了非法阶段名。

`check_outputs.py` 单独运行只返回 `0`（全部一致）或 `1`（存在不一致）；`run_all.py --check`
直接沿用它的返回码。因此判断「对账过没过」只需看 `$?` 是否为 0，不必解析输出文字。
...
| `stress_robustness.py` | 五项稳健性检验：覆盖度 / 窗口 / 缓冲 / 信用区间 / 锚点窗长。报告里的**第六项**（阈值口径交叉核对）不在本脚本，它读 `stress_impact.py` 输出的 `es_1d_99_pct` / `es_hd_99_pct` 两列对读，不另出表 |
...
必须先清空（原因见 §2.6 第 2 条）。重建完想留一份哈希底账的话：

```bash
shasum results/*.csv figures/*.png | sort > /tmp/after.txt
```
...
阶段三三个脚本只读因子表与阶段二的结果 CSV、不重算 VaR，因此在阶段二产物已存在时可独立重跑。
...
不加 `--download` 就完全不联网，直接用已归档的 `raw_data/`。这是默认入口，也是答辩现场最稳的
一条命令。
...
不重跑任何测算，只核对 `results/` 下每张 CSV 的行×列与登记值是否一致，并反向检查有无未登记
的文件。

**其它两条**

```bash
python code/run_all.py --list      # 看执行清单后退出
python code/run_all.py --download  # 连取数一起跑（需联网）
```
...
症状是脚本打印「期望值 vs 实际值」后立刻终止，退出码 1。断言清单与各自的含义见 §7.1 表 12。

定位按三步走：

1. 先看**期望值**的出处。多数是 9/15–9/18 登记的基准值，登记表在 `docs/phase2_spec.md` 与
   `docs/phase2_model_validation.md`。
2. 再看**实际值**是从哪张 CSV 算出来的，单独打开那张 CSV，确认它是本次跑出来的。若行数或
   列数变了，说明上游脚本改了口径，问题在上游，不在报断言的这一处。
3. 上游没变、下游对不上，才是真缺陷。

注意：不得为了跑通而放宽断言（理由见 §7.1）。

### 10.2 产物对账不过

`check_outputs.py` 会逐条列出「实测 N 行 × M 列，期望 N 行 × M 列」以及该期望值的依据
（`[R]` 来自 README 明文声明，`[M]` 由实测补全）。两类成因：

- **改了口径没改登记**：确认新形状是对的之后，同步更新 README「结果目录」两节与
  `code/check_outputs.py` 的 `EXPECTED` 表。
- **出现未登记的 CSV**：`results/` 下多出一张 README 与本表都没写的新表，要么补登记，
  要么确认它本不该被写出来。

### 10.3 缺料中断

症状是 `FileNotFoundError` 或对着一张空表往下算。最常见于「只跑 phase2 或 phase3」：
`results/baseline_var_tr.csv` 由**阶段一**的 `build_tr_factors.py` 产出，阶段二的
`var_backtest.py` 依赖它，跳过阶段一必然中断。改用不带 `--only` 的全量命令即可。

### 10.4 取数失败

`download_*` 失败不会中断全链路（见 §2.3）。若 `raw_data/` 里已有归档数据，后续计算不受影响，
可以直接继续；若确实需要新数据，检查网络后单独重跑对应的 `download_*.py`，
再跑一次全量命令。

### 10.5 想单独重绘某张图

按 §6 表 11 找到该图的产出脚本，直接执行它即可，图随脚本一起更新，不需要重跑全链路。
阶段三的三张图所属脚本只读 CSV，可独立重跑。
